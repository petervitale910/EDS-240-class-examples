...
heatmap_order <- heatmap_data %>% 
  ...
  mutate(subregion_name = fct_reorder(.f = subregion_name,
                                      .x = order))
...
ggplot(heatmap_order, aes(x = year,
  ...
  guides(fill = guide_colorbar(barwidth = 15, barheight = .75, title.position = 'top'))+ # I like guides
  theme(
    legend.position = 'top',
...
ggplot(sbc_subbasin_monthly) +
  ...
  geom_point(aes(x = mean_consum, y = month), 
             ...
             fill = 'white',
             stroke = 2) +
  labs(x = "Mean monthly water availability and consumption (mm)") +
